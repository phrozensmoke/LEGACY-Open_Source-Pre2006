...
"turkish":"tr_" ,
	...
	hwwin=GtkWindow(GTK.WINDOW_TOPLEVEL)
	self.hwwin=hwwin
	hwwin.realize()
	hwwin.set_title("IceWMCP: "+_("Hardware and System Information"))
	hwwin.set_position(GTK.WIN_POS_NONE)
  ...
  # (HELP_MENU+"/_"+BUG_REPORT_MENU+"...", "F5", file_bug_report,5014, ""),
	...
	mainvbox.pack_start(GtkLabel(_("Hardware and System Information")),0,0,4)

	hpaned=GtkHPaned()
	hpaned.set_gutter_size(10)
	hpaned.set_position(315)
	self.hpaned=hpaned
	scrolledleft=GtkScrolledWindow()
	scrolledleft.set_policy(GTK.POLICY_ALWAYS,GTK.POLICY_ALWAYS)
	self.scrolledleft=scrolledleft
	treeleft=hwtree()
	#treeleft.set_usize(275,-2)
	#treeleft.set_line_style(CTREE_LINES_NONE)
	self.treeleft=treeleft
	scrolledleft.add(treeleft)
	hpaned.add(scrolledleft)
	scrolledright=GtkScrolledWindow()
	self.textfield=GtkText()
	self.textfield.set_line_wrap(1)
	self.textfield.set_word_wrap(1)
...
class hwtree(DndCTree.DndCTree):
	...
	mask=None
	    ...
	    try:
		img = gdkpixbuf.new_from_file(iconname)
