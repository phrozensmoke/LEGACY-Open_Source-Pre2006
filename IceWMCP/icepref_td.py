...
import glob,gtk,GTK,re,sys,os,commands,GdkImlib,math,GDK,icepref_search
...
class Label(GtkLabel):
	def __init__(self, title,sstopper=30):
		GtkLabel.__init__(self, breakTitle(title,sstopper))
...
class Entry(GtkVBox):
	...
	def init_widgets(self, title):
		...
		self.pack_start(GtkLabel(""),1,1,0)
...
class ImagePreviewer:
	...
	def update_image(self,image_file):
		...
		try:
			...
			myim=GdkImlib.Image(self.current_image)
...
class ButtonEntry(GtkVBox):
	...
	def init_widgets(self, title):
		...
		self.pack_start(hbox, FALSE, FALSE, 0)
		vbsmall.pack_start(GtkLabel(""), 1, 1, 0)
		...
		hbhide=GtkVBox(0,0)
		hbhide.pack_start(GtkLabel(""),1,1,0)
...
class Color(ButtonEntry):
	def select(self, data=None):
		value = self.entry.get_text()
		self.win = GtkColorSelectionDialog(name=_('Select Color'))
...
class Font(ButtonEntry):
	...
	def select(self, data=None):
		self.win = GtkFontSelectionDialog(_('Select Font'))
		self.win.ok_button.connect('clicked', self.ok)
		self.win.cancel_button.connect('clicked', self.cancel)
		value = self.entry.get_text()
		if value !='':
...
class ThemeSel(GtkVBox):
	...
	def init_widgets(self, title):
		...
		clist = GtkCList(2, [_('Theme Name')+'               ', _('File')+'   '])
...
class Application(GtkWindow):
	...
	def __init__(self, argv):
		GtkWindow.__init__(self, title='IcePref2: '+_('Theme Designer'))
		global APP_WINDOW
		APP_WINDOW=self
		self.my_last_file=''
		...
		self.vbox1=GtkVBox(0,0)
		self.vbox = GtkVBox(0,4)
		...
		try:
			...
			self.vbox.pack_start(GtkLabel("IcePref2"),0,0,1)
		self.vbox.pack_start(GtkLabel(_("Theme Designer")),0,0,1)
		self.init_notebook()
		self.init_buttons()
		self.set_default_size(gtk.screen_width()-100, gtk.screen_height()-150)
		self.stat_label=GtkLabel("IcePref2 "+_("Theme Designer")+" "+VERSION)
	...
	def init_notebook(self):
		...
		for tab in TABS:
			...
			#scroll.set_usize(gtk.screen_width()-150, gtk.screen_height()-290)
					...
					if (item=="ThemeAuthor"): 
						if not ts==None: 
							...
							if not ts.iprev.entry_box==None:
								...
								ts.iprev.entry_box.pack_start(GtkLabel("  "),1,1,7)
	...
	def widget_chooser(self, item,Image_prev=None):
		...
		for ichild in widget.children():
			TIPS.set_tip(ichild,W_TITLE+"\n["+item+"]")
			try:
				for ichildd in ichild.children():
					if not ichildd.__class__.__name__=="GtkButton": TIPS.set_tip(ichildd,W_TITLE+"\n["+item+"]")
			except:
				pass
		return widget
	...
	def newThemeAsk(self,*args):
		...
		v.pack_start(GtkLabel(_("Create A New IceWM Theme")),0,0,3)
		etable = GtkTable (3, 4, 0)
		etable.attach( GtkLabel(_("Theme Name")+": "), 0, 1, 0, 1, (GTK.FILL), (0), 2, 0)
		etable.attach( GtkLabel(_("Theme Author")+": "), 0, 1, 1, 2, (GTK.FILL), (0), 2, 0)
		etable.attach( GtkLabel(_("Theme Description")+": "), 0, 1, 2, 3, (GTK.FILL), (0), 2, 0)
		etable.attach( GtkLabel(_("Directory")+": "), 0, 1, 3, 4, (GTK.FILL), (0), 2, 0)
		entries=[GtkEntry(),GtkEntry(),GtkEntry(),GtkEntry()]
		...
		try:
			...
			dirbutton.add(GtkLabel(" "+_("Select")+"... "))
