...
class wallwin:
  ...
  (_("/_Help")+"/_"+BUG_REPORT_MENU+"...", "F5", file_bug_report,5004, ""),
	...
	mainvbox.pack_start(GtkLabel(_("Window Options")),0,0,2)	
	hbox1=GtkHBox(0,0)
	hbox1.set_spacing(5)
	sc=GtkScrolledWindow()
	self.winlist=GtkCList(2,[' '+'wm_class',' '+'wm_name'])
	self.sc=sc
	sc.add(self.winlist)
	sc.set_usize(300,-2)
	...
	h1=GtkHBox(0,0)
	h1.set_spacing(3)
	h1.pack_start(GtkLabel(_("Icon: ")),0,0,0)
	self.iconentry=GtkEntry()
	h1.pack_start(self.iconentry,1,1,0)
	browsebutt=GtkButton()
	try:
			browsebutt.add(  loadScaledImage(getPixDir()+"ism_load.png",24,24)  )
	except:
			browsebutt.add(GtkLabel(_(" Browse... ")))
	...
	h3=GtkHBox(0,0)
	h3.set_spacing(3)
	h3.pack_start(GtkLabel(_("Layer: ")),0,0,0)
	laycom=GtkCombo()
	...
	h4=GtkHBox(0,0)
	h4.set_spacing(3)
	h4.pack_start(GtkLabel(_("Geometry: ")),0,0,0)
	geoentry=GtkEntry()
	...
	h5=GtkHBox(0,0)
	h5.set_spacing(3)
	h5.pack_start(GtkLabel(_("Tray Icon: ")),0,0,0)
	traycom=GtkCombo()
	...
	maxx=layx
	for ii in worder:
		ck=GtkCheckButton(_(self.winhints[ii]))
    ...
    def update_image(self,image_file):
		#print image_file
		for ii in self.prevlay.children():
			try:
	...
	mb.pack_start(GtkHSeparator(),0,0,4)
	hb=GtkHBox(0,0)
	hb2=GtkHBox(0,0)
	wmctext=GtkEntry()
	wmntext=GtkEntry()
	hb.pack_start(GtkLabel("WM_CLASS:  "),0,0,0)
	hb.pack_start(wmctext,1,1,0)
	mb.pack_start(hb,1,1,1)
	hb2.pack_start(GtkLabel("WM_NAME:   "),0,0,0)
