...
sqlite = "sqlite"
sources = ["./sqlite/_sqlite.c", "./sqlite/encode.c", "./sqlite/strsep.c"]
macros = []

if sys.platform in ("linux-i386", "linux2"): # most Linux
    include_dirs = ['/usr/include/sqlite','./sqlite']
    library_dirs = ['/usr/lib/','./sqlite']
    libraries = [sqlite]
    runtime_library_dirs = []
    extra_objects = []
elif sys.platform in ("freebsd4", "freebsd5", "openbsd2", "cygwin"):
    LOCALBASE = os.environ.get("LOCALBASE", "/usr/local")
    include_dirs = ['%s/include' % LOCALBASE,'./sqlite']
    library_dirs = ['%s/lib/' % LOCALBASE,'./sqlite']
    libraries = [sqlite]
    runtime_library_dirs = []
    extra_objects = []
    ...
    library_dirs = ['/usr/local/lib','./sqlite']
    libraries = [sqlite]
