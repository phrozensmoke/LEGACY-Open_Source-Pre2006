...
static void do_modem_info_query(hd_data_t *hd_data, hd_t *hd, ser_device_t *sm);
...
void get_serial_modem(hd_data_t *hd_data)
{
  ...
  usleep(250000);		/* PnP protocol; 200ms seems to be too fast  */
  ...
  for(sm = hd_data->ser_modem; sm; sm = sm->next) {
    ...
    if(!sm->is_modem) {  // some other serial device, non-modem - reset and continue
	...
	do_modem_info_query(hd_data, hd,sm);
...
void do_modem_info_query(hd_data_t *hd_data, hd_t *hd,ser_device_t *modem) {
  /* just for testing - NOPE - changed here - PhrozenSmoke, do real modem query instead */
   if(hd && hd->base_class.id == bc_modem) {
    hd_res_t *res;
    ser_device_t *sm;
    int i;
  ...
  (sm = modem)->do_io = 1;
    ...
    res = add_res_entry(&hd->res, new_mem(sizeof *res));
      res->modemq.type = res_modem_query;      

    for(i = 0; i < sizeof cmds / sizeof *cmds; i++) {
      sprintf(at, "ATI%d\r", cmds[i]);
      sprintf(at2, "ATI%d", cmds[i]);
      at_cmd(hd_data, at, 0, 1);
      slq = sm->at_resp;
      if(slq && !strcmp(slq->str, at2)) {slq = slq->next; }
       if(slq) { 
	if (i==0) {res->modemq.ati0=new_str(slq->str); }
	if (i==1) {res->modemq.ati1=new_str(slq->str); }
	if (i==2) {res->modemq.ati2=new_str(slq->str); }
	if (i==3) {res->modemq.ati3=new_str(slq->str); }
	if (i==4) {res->modemq.ati4=new_str(slq->str); }
	if (i==5) {res->modemq.ati5=new_str(slq->str); }
	if (i==6) {res->modemq.ati6=new_str(slq->str); }
	if (i==7) {res->modemq.ati7=new_str(slq->str); }
		}  else { 
	if (i==0) {res->modemq.ati0=new_str("no response"); }
	if (i==1) {res->modemq.ati1=new_str("no response"); }
	if (i==2) {res->modemq.ati2=new_str("no response");}
	if (i==3) {res->modemq.ati3=new_str("no response"); }
	if (i==4) {res->modemq.ati4=new_str("no response"); }
	if (i==5) {res->modemq.ati5=new_str("no response");}
	if (i==6) {res->modemq.ati6=new_str("no response"); }
	if (i==7) {res->modemq.ati7=new_str("no response"); }
...
void guess_modem_name(hd_data_t *hd_data, ser_device_t *modem)
{
 ...
 /*   addedd - too many modems being reported as a 'deskporte' modem, try ATI4 first */
  /* ATI4 command */
  at_cmd(hd_data, "ATI4\r", 0, 1);
  sl = sm->at_resp;
  if(sl && !strcmp(sl->str, "ATI4")) sl = sl->next;	/* skip AT cmd echo */

  if(sl) {
    /* printf("\n\nMODEM STRING: %s\n\n",sl->str);
    fflush(stdout);  */
    if(strstr(sl->str, "Modem") || strstr(sl->str, "Voice") || strstr(sl->str, "Fax")) {
      /* looks like a what the modem is suppose to say  */

      if (strstr(sl->str, "Best Data")) {sm->vend = new_str("Best Data"); } else { sm->vend = new_str(sl->str); }
      sm->user_name = new_str(sl->str);
  ...
  if(sl) {
    ...
    if(strstr(sl->str, "RCV56DPF-PLL L8571A")) {
      ...
      sm->vend = new_str("Microcom");
      sm->user_name = new_str("DeskPorte 56K Voice (or similiar)");
