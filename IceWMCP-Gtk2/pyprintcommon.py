...
def getImageButton (picon,btext,windowval) :  
        b=gtk.Button()
        b.add(getImageVBox(picon,btext,windowval))
        b.show_all()        
        return b

def loadImage(picon,windowval):
        try:
                p=gtk.Image(windowval,getPixDir()+str(picon),None)
                p.show_all()
                return p
        except:
                return None

def getImageVBox(picon,btext,windowval):
        try:
          ...
          v.pack_start(gtk.Label(str(btext)),0,1,1)
          v.show_all()
          return v
        except:
          v=gtk.HBox(1,1)
          v.pack_start(gtk.Label(str(btext)),1,1,1)
          v.show_all()
          return v
