...
BUG_REPORT_ADDRESS="djm6202@yahoo.co.nz"   # who to send bug reports to
...
def getSysInfo(*args): # gather useful system information for bug-reporting, info users always fail to send
        ...
        sys_info=sys_info+"gtk.gdkPIXBUF VERSION : "+str(os.popen("gdk-pixbuf-config --version").readline()) +"\n\n"
...
def file_bug_report(app_num=5000,*args):
        ...
        if not p: p=gtk.Label(wintitle)
        mainvbox.pack_start(p,0,0,0)
        mainvbox.pack_start(gtk.Label(_("Bug Report")),0,0,2)
        mainvbox.pack_start(gtk.HSeparator(),0,0,2)
        table1 = gtk.Table (3, 2, 0)
        table1.set_col_spacings (8)
        table1.set_row_spacings (2)
        table1.set_border_width (4)
        table1.attach(gtk.Label(_("To")+":"), 0, 1, 0, 1, (GTK.FILL), (0), 0, 0)
        table1.attach(gtk.Label(_("From")+":"), 0, 1, 1, 2, (GTK.FILL), (0), 0, 0)
        table1.attach(gtk.Label(_("SMTP Server")+":"), 0, 1, 2, 3, (GTK.FILL), (0), 0, 0)
        table1.attach(gtk.Label("<"+BUG_REPORT_ADDRESS+">"), 1, 2, 0, 1, (GTK.EXPAND+GTK.FILL), (0), 0, 0)
        sserver=gtk.Entry()
        sfrom=gtk.Entry()
        ...
        mainvbox.pack_start(gtk.Label(_("Please describe the bug or problem in detail")+":"),0,0,4)
        sc=gtk.ScrolledWindow()
        sctext=gtk.TextView()
        sctext.set_editable(gtk.TRUE)
        sctext.set_wrap_mode(gtk.WRAP_WORD)
        sctext.get_buffer().set_text(app_name)   # for testing
        sc.add(sctext)
        mainvbox.pack_start(sc,1,1,0)
        mainvbox.pack_start(gtk.Label(_("The following system information will be sent with your bug report")+":"),0,0,6)
        sc1=gtk.ScrolledWindow()
        sctext1=gtk.Text()
        sctext1.set_editable(0)
        sctext1.get_buffer().set_text(getSysInfo())
        sctext1.set_wrap_mode(gtk.WRAP_WORD)
        sctext1.set_line_wrap(1)
        sc1.add(sctext1)
        mainvbox.pack_start(sc1,1,1,0)
        sendbutt=gtk.Button(_("SEND NOW"))
