...
class controlwin:
    def __init__(self,use_loader=0) :
        ...
        controlwin   = gtk.Window(gtk.WINDOW_TOPLEVEL)
  ...
  (_("/_Help")+"/_"+BUG_REPORT_MENU+"...", "F5", file_bug_report,5000, ""),
        ...
        titlebar = gtk.HBox(0,0)
        titlebar.set_spacing(0)
        mainvbox.pack_start(titlebar,0,0,0)
        titlebar.pack_start(gtk.Label(" "),1,1,0)
        titlebar.pack_start(getImage(getBaseDir()+"icewmcp.png",
                                     "IceWM Control Panel"),0,0,0)
        titlebar.pack_start(gtk.Label(" "),1,1,0)

        try:   # added 5.6.2003 - add a 'button box' for changing views
            buttonbox = gtk.VBox(0,0)
            buttonbox.set_spacing(0)
            for buttons in  ( 
               ('view_icon.png',_("/_View/_Icon View"),self.iconView) , 
               ('view_list.png',_("/_View/_List View"), self.listView) , 
               ('view_col.png',_("Column View"),self.columnView)):
                ...
                bicon.set_relief(gtk.RELIEF_NONE)
    ...
    def showConfig(self,*args) :
        ...
        global WMCLASS
        global WMNAME
        confwin = gtk.Window(gtk.WINDOW_TOPLEVEL)
        ...
        mainvbox.pack_start(gtk.Label(_("  Choose which icons to hide or show:  ")),0,0,3)
        sc = gtk.ScrolledWindow()
        gl = gtk.Layout()
        max_x = 5
        max_y = 3
        ...
        hb2 = gtk.HBox(0,0)
        hb2.set_spacing(10)
        sbutton = gtk.Button(_("Save"))
        TIPS.set_tip(sbutton,_("Save"))
        cbutton = gtk.Button(DIALOG_CANCEL)
...
#                    pass
    ...
    def checkAvail(self,to_run) :  # added, 5/6/2003 - check for availability of applet programs
        ...
        if appc.startswith("python "): # 'internal' applets
            ...
            # we can check for the 'existence' if internal applets on static
            # binary versions of IceWMCP, because the actual .py files are
            # embedded in the 'icewmcp' executable, and don't exist as actual
            # 'files' on the file system.  The alternatives are to: 1. assume
            # all 'internal' applets exist   or 2. implement some expensive
            # 'import' test, which will greatly slow down the launching of the
            # applet and possibly create unknown/unwanted side effects....so,
            # for static-binary versions, we will simply ASSUME that all
            # internal applets 'exist' - since there is no way for the use to
    ...
    def getCPIcon(self,micon,mdesc,mexec,mtooltip,listvw = 0) :
        ...
        relief = 2
        try:
            if listview == 1: MYFONT=load_font("-*-helvetica-medium-r-normal-*-14-*") 
            else: MYFONT = load_font("-*-helvetica-medium-r-normal-*-12-*") 
        except:
            pass
        if p:
            if listview == 1:  v=gtk.HBox(0,0)
            else: v = gtk.VBox(0,0)
            v.set_spacing(1)
            v.pack_start(p,0,0,0)
            if listview == 1:         labname=gtk.Label("  "+str(mdesc).replace("\n"," ").replace("\r"," ").replace("\t"," ").replace("  "," ").replace(" -","-").replace("- ","-").strip())
            else:           labname = gtk.Label(str(mdesc).strip())
            try:
            ...
            elif listvw == 2: v.set_size_request(v.size_request()[0]+5,v.size_request()[1]+3)
            ...
            if listview == 1: b=gtk.Button("  "+str(mdesc).replace("\n"," ").replace("\r"," ").replace("\t"," ").replace("  "," ").replace(" -","-").replace("- ","-").strip())
            else: b = gtk.Button(str(mdesc).strip())
            try:
