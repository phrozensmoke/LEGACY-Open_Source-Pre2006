...
class PicMenuItem(gtk.MenuItem):
    ...
    def __init__(self, text, pix, mask):
        ...
        if (pix, mask) != (None, None):
            ...
            icon.show()
            hbox.pack_start(icon, gtk.FALSE, gtk.FALSE)
        if text:
...
def test():
    img = gtk.Image()
    img.set_from_file("pixmaps/iceme_empty.png")
    pix, mask = img.render_pixmap_and_mask()
    tree = IceMenuTree(self)
    win = PreviewWindow(tree, pix, mask)
    win.show()
    gtk.mainloop()
