...
class mousetab:
    def __init__(self) :
        ...
        self.adj=gtk.Adjustment(4,1,75,1,1,1)
        c=gtk.HScale(self.adj)
        ...
        hb.set_spacing(4)
        mainvbox.pack_start(gtk.Label(""),1,1,0)
    ...
    def doSafe(self,*args):
        ...
        mainv.pack_start(gtk.Label(_("Your mouse speed has been set to '")+str(int(self.adj.value))+_("'.\nIf you are not happy with this mouse speed,\nwait 7 seconds and the mouse speed will be\nreset to a reasonable speed.\n")),0,0,2)
        hb=gtk.VBox(0,0)
        hb.set_spacing(7)
        b1=gtk.Button(_("CANCEL change and RESET the mouse"))
        b2=gtk.Button(_("KEEP the new mouse speed"))
...
class cursortab:
    def __init__(self,cwin):
        ...
        for ii in self.cursors:
            ...
            cwid=icepref_td.ImageWidget(ii, 
                                        "\""+ii+"\"",
...
class mousewin:
  ...
  (_("/_Help")+"/_"+BUG_REPORT_MENU+"...", "F5", file_bug_report,5012, ""),
        ...
        notebook.append_page(curspan.mainvbox, gtk.Label(translateP("Cursors")))
