...
{
return;  /* debugging disabled: PhrozenSmoke */
#ifdef DETAILED_OUT
    if(LOADER_DEBUG)
    {
	FILE* f;
...
void* LookupExternal(const char* library, int ordinal)
{
    ...
    // printf("External func %s:%d\n", library, ordinal); /* disabled: PhrozenSmoke */
  ...
  if (!strcmp(library, "tsd32.dll"))
	...
	/* printf("External dll loaded (offset: 0x%x, func: %p)\n",
	       hand, func); */ /*disabled PhrozenSmoke */
	return func;
    }
...
void my_garbagecollection(void)
{
    ...
    /* printf("Total Unfree %d bytes cnt %d [%p,%d]\n",unfree, unfreecnt, last_alloc, alccnt); */  /* disabled: PhrozenSmoke */
#endif
    g_tls = NULL;
    list = NULL;
}
