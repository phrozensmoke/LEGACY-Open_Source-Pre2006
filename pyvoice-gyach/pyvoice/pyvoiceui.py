...
# Support my personal mic tricks which will not be made public
# Used only if available
global HAVE_MIC_TRICKS
HAVE_MIC_TRICKS=0
try:
	import pyvoice_tricks
	pyvoice_tricks.PYVOICE_LIB=pyvoice
	pyvoice_tricks.gtk=gtk
	HAVE_MIC_TRICKS=1
except:
	HAVE_MIC_TRICKS=0
...
def _(somestr):
	return to_utf8(pyvoice_xtext.gettext(somestr))
...
class pyvoicegui:
	...
	self.aliases={}  # alias backup
	TIPS=Tooltips()
	pyvoicewin=Window(WINDOW_TOPLEVEL)
	pyvoicewin.set_wmclass(WMCLASS,WMNAME)
	pyvoicewin.realize()
	pyvoicewin.set_title(_("pY! Voice Chat"))
	pyvoicewin.set_position(WIN_POS_CENTER)
  ...
  (_("/_Help/_Contributors..."), "F5", self.doContrib,17, ""),
	...
	else: mainvbox.pack_start(Label(_("pY! Voice Chat")),0,0,5)
	...
	myvolbox=Button(" "+_("Test")+" ")
	#myvolbox.connect("clicked",startMicTest)
	myvolbox.connect("clicked",self.tstartMicTest)
	self.myvolbox=myvolbox
	talkbox.pack_start(myvolbox,0,0,0)
	talkbox.pack_start(Label("    "),0,0,0)
	mymonitor=ColorButton()  # will hold our color indicator, DrawingArea?
	...
	sc=ScrolledWindow()
	sc.set_size_request(-1,165)
	self.nmlist=CList(3,[' I ',' '+_('Name')+' ',' '+_('Alias')+' '])
	self.nmlist.set_column_width(0,16)
	if HAVE_MIC_TRICKS==1: self.nmlist.set_column_width(0,24)
	...
	if HAVE_MIC_TRICKS==1: 
		bootbox=HBox(1,0)
		bootone=Button(_("Boot"))
		bootmany=Button(_("Keep Booting"))
		bootstop=Button(_("Stop Booting"))
		bootbox.pack_start(bootone,1,1,2)
		bootbox.pack_start(bootmany,1,1,2)
		bootbox.pack_start(bootstop,1,1,2)
		bootone.connect("clicked",self.boot_once_cb )
		bootmany.connect("clicked",self.boot_many_cb )
		bootstop.connect("clicked",self.stop_booting_cb)
		mainvbox.pack_start(bootbox,0,0,1)
	...
	pyvoicewin.set_default_size(300,400)
    ...
    if HAVE_MIC_TRICKS==1:
	def boot_once_cb(self,*args):
		# once time mic boot
		if self.selected_name=="": return
		pyvoice_tricks.boot_once(self.selected_name)
	def boot_many_cb(self,*args):
		# repeated mic boot
		if self.selected_name=="": return
		pyvoice_tricks.boot_many(self.selected_name)
	def stop_booting_cb(self,*args):
		if self.selected_name=="": return
		pyvoice_tricks.stop_booting(self.selected_name)


    def tstartMicTest(self,*args):
	#threads_enter()
	#threads_enter()
	startMicTest()
	#threads_leave()
	...
	if user_is_ignored(self.selected_name):
		...
		s=s+_("Ignored")+":  "+_("No")+"\n"
	s=s+_("Voice Packets Sent")+":  "+ to_utf8(get_stats_for_user(self.selected_name))
	message( s, ['  '+_('OK')+'  '])
	...
	self.users.clear()
	for ii in users:
		if self.aliases.has_key(ii): 
			self.users[ii]=self.aliases[ii]
		else:
			self.users[ii]=ii
	...
	try:
		...
		for ii in flist:
			...
			self.aliases[pair[0].strip()]=pair[1].strip()
			if self.users.has_key(pair[0].strip()): 
				self.users[pair[0].strip()]=pair[1].strip()
	except:
		pass
	...
	for ii in lus:
		...
		if HAVE_MIC_TRICKS==1:
			if pyvoice_tricks.is_booting(ii): igged=igged+"B"
		k=[to_utf8(igged),to_utf8(ii),to_utf8(self.users[ii])]
	...
	if self.users.has_key(i):
		self.update_name(to_utf8(i))
		self.update_alias(to_utf8(self.users[i]))
	elif i==_("MIC TEST"):
		self.update_name(i)
	...
	self.roomentry.set_text(to_utf8(i))
	self.last_room=i
	return 1
	...
	mains.set_spacing(3)
	mains.pack_start( Label(_("Select a voice server")+":"), 0,0,2)
	mainc=Combo()
	...
	mains.pack_start(HSeparator(), 0,0,2)
	b=Button(" "+_("OK")+" ")
...
def message(message="", buttons=("Ok",),modal=1, big=0):
    ...
    if big==1: win.set_default_size(375,-1)
    win.show_all()
    if modal==1:
    	mainloop()
    	return win.ret
...
def loadImage(picon,windowval):
	...
	"""
    	try:
        	p=Image()
		p.set_from_file(getPixDir()+str(picon))
       		p.show_all()
        	return p
    	except:
        	return None
...
def getIconButton(winval,icon,label):
	...
	if ic==None: return Button(label)
	b=Button()
	b.add(ic)
	b.show_all()
	return b
def getIconButton2(winval,icon,label, toggle=0):
	...
	ic=loadImage(icon,winval)
	if ic==None: 
		if toggle==1: return ToggleButton(label)
		return Button(label)
	if toggle==1: b=ToggleButton(None)
	else: b=Button()
	hb=HBox(0,0)
	hb.set_border_width(1)
	hb.set_spacing(3)
	hb.pack_start(ic,0,0,0)
	hb.pack_start(Label(label),1,1,0)
	b.show_all()
	b.add(hb)
	return b


class ColorButton(HBox):
	"""
	...
	def setColor(self,*args):
		...
		if not self.drawwin.get_colormap(): return
		...
		self.drawwin.draw_rectangle(self.gc,TRUE,0,0,self.drawwin.get_size()[0],self.drawwin.get_size()[1])
...
class SoundBar(HBox):
	...
	def __init__(self):
		...
		while i<10:
			drawing_area = DrawingArea()
			drawing_area.set_size_request(10, 6)
			drawing_area.show() 
			self.drawing_area=drawing_area
			self.color_i16[i]=[0.10,0.10,0.11]
			self.drawwins[i]=drawing_area.window
			self.gcs[i]=None
	...
	def updateColor(self,some_block):
		i=some_block
		if not self.drawwins[i]: self.drawwins[i]=self.drawings[i].window
		if not self.gcs[i]: self.gcs[i]=self.drawwins[i].new_gc()
		if not self.drawwins[i].get_colormap(): return
		...
		self.drawwins[i].draw_rectangle(self.gcs[i],TRUE,0,0,10,6)
